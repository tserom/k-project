...
更细的端口、环境变量与依赖关系见 [docs/WORKSPACE.md](./docs/WORKSPACE.md)。微前端如何拼在一起见 [docs/MICROFRONTEND.md](./docs/MICROFRONTEND.md)。目录搬迁说明见 [docs/REPO_LAYOUT.md](./docs/REPO_LAYOUT.md)。

## 本地开发（不用 Docker）

1. **MySQL**：按 [apps/user-backend/README.md](./apps/user-backend/README.md) 创建库并配置 `.env`。
2. **后端**：`cd apps/user-backend && make run`（默认 `:8080`）。
3. **子应用**：在 `apps/hello-front`、`apps/user-front` 分别 `pnpm install && pnpm dev`（端口 `8100` / `8101`）。
4. **父应用**：`cd apps/host`，确认 `.env.development` 里子应用 URL 与上一步端口一致，再 `pnpm dev`（默认 `8000`）。
