...
参考文档：`docs/MICROFRONTEND.md`、`docs/WORKSPACE.md`（端口唯一信息源）。
...
Host 按导航项 `subAppBusName` 派发 **`{subAppBusName}-route`**。`instanceId` 用于多实例 tab，子应用只处理匹配自己的 payload。
...
Docker/网关下同源；多端口 dev 可在 `vite.config` 配 proxy。
